CREATE OR REPLACE FUNCTION create_booking_from_template(
  p_template_id UUID,
...
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER -- Runs as owner (postgres) to bypass RLS on sessions creation
AS $$
DECLARE
  v_session_id UUID;
  v_template RECORD;
